...
func init() {
	unpack.RegisterFormat("PECompact", PECompact{})
}
...
func (PECompact) Detect(path string) int {
	...
	if err != nil {
		return PECOMPACT_VER_NONE
	}
	defer peFile.Close()

	// Check for PECompact section
	hasPECompactSection := false
	for _, section := range peFile.Sections {
		...
		nameStr := string(name)
		if nameStr == ".pec" || nameStr == ".PEC" || nameStr == "PEC" ||
			nameStr == ".pec2" || nameStr == ".PEC2" {
			hasPECompactSection = true
			break
		}
	}

	if hasPECompactSection {
		return PECOMPACT_VER_1
	}

	// Check for PECompact signature in file
	f, err := os.Open(path)
	if err != nil {
		return PECOMPACT_VER_NONE
	}
	...
	// Read first 64KB to find PECompact signature
	...
	// Check for PECompact signatures
	for i, sig := range pecompactSignatures {
		if bytes.Contains(buf[:n], sig) {
			return PECOMPACT_VER_1 + i
		}
	}

	// Check entry point code for PECompact characteristics
	ep := getEP(path)
	if ep > 0 {
		epbuff := make([]byte, 512)
		_, err = f.ReadAt(epbuff, int64(ep))
		if err == nil {
			// PECompact often has specific patterns in entry point
			if hasPECompactPattern(epbuff) {
				return PECOMPACT_VER_1
			}
		}
	}
...
// hasPECompactPattern checks for PECompact-specific patterns in entry point code
func hasPECompactPattern(epbuff []byte) bool {
	// PECompact entry point often contains specific instruction sequences
	if len(epbuff) < 10 {
		return false
	}
	...
	// Pattern 2: Specific byte sequences common in PECompact
	pecompactPatterns := [][]byte{
		{0x60, 0xE8},           // PUSHAD, CALL
		{0x9C, 0x60},           // PUSHFD, PUSHAD
		{0xE8, 0x00, 0x00, 0x00, 0x00}, // CALL with zero offset
	}

	for _, pattern := range pecompactPatterns {
...
func (PECompact) Unpack(path string) (io.ReaderAt, error) {
	...
	oepRVA := findPECompactOEP(epbuff, peFile)
	if oepRVA == 0 {
	...
	// Fix import table
	fixPECompactImportTable(unpackedData, peFile, peSigOffset)
...
// findPECompactOEP finds OEP from PECompact entry point code
func findPECompactOEP(epbuff []byte, peFile *pe.File) uint32 {
	// PECompact stores OEP in various patterns
...
// fixPECompactImportTable attempts to fix the import table
func fixPECompactImportTable(unpackedData []byte, peFile *pe.File, peSigOffset int) {
...
func createTempFileFromData(data []byte) (io.ReaderAt, error) {
	tmpFile, err := os.CreateTemp("", "unpack_pecompact_*.exe")
	if err != nil {
		return nil, err
	}
