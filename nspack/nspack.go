...
func init() {
	unpack.RegisterFormat("NSPack", NSPack{})
}

const (
	NSPACK_VER_NONE = 0 + iota
	NSPACK_VER_1
	NSPACK_VER_2
)

// NSPack signatures
var nspackSignatures = [][]byte{
	[]byte("NSPack"),
	[]byte("NSPACK"),
	[]byte("NSP0"),
	[]byte("NSP1"),
...
func (NSPack) Name() string {
	return "NSPack"
}

// Detect checks if the file is packed with NSPack
func (NSPack) Detect(path string) int {
	peFile, err := pe.Open(path)
	if err != nil {
		return NSPACK_VER_NONE
	}
	defer peFile.Close()

	// Check for NSPack section
	hasNSPackSection := false
	for _, section := range peFile.Sections {
		...
		nameStr := string(name)
		if nameStr == ".nsp0" || nameStr == ".nsp1" || nameStr == ".NSP0" ||
			nameStr == ".NSP1" || nameStr == "NSP0" || nameStr == "NSP1" {
			hasNSPackSection = true
			break
		}
	}
	...
	// Check for NSPack signature in file
	f, err := os.Open(path)
	if err != nil {
		return NSPACK_VER_NONE
	}
	...
	// Read first 64KB to find NSPack signature
	...
	// Check for NSPack signatures
	for i, sig := range nspackSignatures {
		if bytes.Contains(buf[:n], sig) {
			return NSPACK_VER_1 + i
		}
	}

	// Check entry point code for NSPack characteristics
	ep := getEP(path)
	if ep > 0 {
		epbuff := make([]byte, 512)
		_, err = f.ReadAt(epbuff, int64(ep))
		if err == nil {
			// NSPack often has specific patterns in entry point
			if hasNSPackPattern(epbuff) {
				return NSPACK_VER_1
			}
		}
	}
...
// hasNSPackPattern checks for NSPack-specific patterns in entry point code
func hasNSPackPattern(epbuff []byte) bool {
	// NSPack entry point often contains specific instruction sequences
	if len(epbuff) < 10 {
		return false
	}
	...
	// Pattern 2: Specific byte sequences common in NSPack
	nspackPatterns := [][]byte{
	...
	for _, pattern := range nspackPatterns {
...
// Unpack unpacks a NSPack-packed PE file
func (NSPack) Unpack(path string) (io.ReaderAt, error) {
	peFile, err := pe.Open(path)
	if err != nil {
		return nil, err
	}
	...
	oepRVA := findNSPackOEP(epbuff, peFile)
	if oepRVA == 0 {
	...
	// Fix import table
	fixNSPackImportTable(unpackedData, peFile, peSigOffset)
...
// findNSPackOEP finds OEP from NSPack entry point code
func findNSPackOEP(epbuff []byte, peFile *pe.File) uint32 {
	// NSPack stores OEP in various patterns
...
// fixNSPackImportTable attempts to fix the import table
func fixNSPackImportTable(unpackedData []byte, peFile *pe.File, peSigOffset int) {
...
func createTempFileFromData(data []byte) (io.ReaderAt, error) {
	tmpFile, err := os.CreateTemp("", "unpack_nspack_*.exe")
	if err != nil {
		return nil, err
	}
