...
func init() {
	unpack.RegisterFormat("Themida", Themida{})
}

const (
	THEMIDA_VER_NONE = 0 + iota
	THEMIDA_VER_1
	THEMIDA_VER_2
)

// Themida signatures and characteristics
var themidaSignatures = [][]byte{
	[]byte("Themida"),
	[]byte("THEMIDA"),
	[]byte("themida"),
	[]byte("This program protected with Themida"),
	[]byte("This program protected with THEMIDA"),
}

type Themida struct{}

func (Themida) Name() string {
	return "Themida"
}

// Detect checks if the file is packed with Themida
func (Themida) Detect(path string) int {
	peFile, err := pe.Open(path)
	if err != nil {
		return THEMIDA_VER_NONE
	}
	defer peFile.Close()

	// Check for Themida section
	hasThemidaSection := false
	for _, section := range peFile.Sections {
		...
		nameStr := string(name)
		if nameStr == ".themida" || nameStr == "Themida" || nameStr == "THEMIDA" ||
			nameStr == ".vmp" || nameStr == ".vmp0" || nameStr == ".vmp1" {
			hasThemidaSection = true
			break
		}
	}

	if hasThemidaSection {
		return THEMIDA_VER_1
	}

	// Check for Themida signature in file
	f, err := os.Open(path)
	if err != nil {
		return THEMIDA_VER_NONE
	}
	...
	// Read first 64KB to find Themida signature
	...
	for i, sig := range themidaSignatures {
		if bytes.Contains(buf[:n], sig) {
			return THEMIDA_VER_1 + i
		}
	}
	...
	// Check entry point code for Themida characteristics
	ep := getEP(path)
	if ep > 0 {
		epbuff := make([]byte, 512)
		_, err = f.ReadAt(epbuff, int64(ep))
		if err == nil {
			// Themida often has specific patterns in entry point
			if hasThemidaPattern(epbuff) {
				return THEMIDA_VER_1
			}
		}
	}
...
// hasThemidaPattern checks for Themida-specific patterns in entry point code
func hasThemidaPattern(epbuff []byte) bool {
	// Themida entry point often contains specific instruction sequences
	if len(epbuff) < 10 {
		return false
	}

	// Pattern 1: PUSHAD (0x60) followed by complex unpacking code
	if epbuff[0] == 0x60 { // PUSHAD
		// Look for CALL instruction nearby
		for i := 1; i < len(epbuff)-5 && i < 30; i++ {
	...
	// Pattern 2: Specific byte sequences common in Themida
	themidaPatterns := [][]byte{
		{0x60, 0xE8},           // PUSHAD, CALL
		{0x9C, 0x60},           // PUSHFD, PUSHAD
		{0x60, 0xBE},           // PUSHAD, MOV ESI
		{0xE8, 0x00, 0x00, 0x00, 0x00}, // CALL with zero offset
		{0xEB, 0x00},           // JMP short (common in packers)
	}

	for _, pattern := range themidaPatterns {
...
// Unpack unpacks a Themida-packed PE file
// Note: Themida is a very advanced protector, full unpacking requires
// complex techniques including VM analysis. This is a basic implementation.
func (Themida) Unpack(path string) (io.ReaderAt, error) {
	peFile, err := pe.Open(path)
	if err != nil {
		return nil, err
	}
	...
	// Themida uses advanced protection, so finding OEP is complex
	oepRVA := findThemidaOEP(epbuff, peFile)
	if oepRVA == 0 {
	...
	// Fix import table
	fixThemidaImportTable(unpackedData, peFile, peSigOffset)
...
// findThemidaOEP finds OEP from Themida entry point code
func findThemidaOEP(epbuff []byte, peFile *pe.File) uint32 {
	// Themida uses advanced protection, so finding OEP is complex
...
// fixThemidaImportTable attempts to fix the import table
func fixThemidaImportTable(unpackedData []byte, peFile *pe.File, peSigOffset int) {
...
func createTempFileFromData(data []byte) (io.ReaderAt, error) {
	tmpFile, err := os.CreateTemp("", "unpack_themida_*.exe")
	if err != nil {
		return nil, err
	}
