...
func init() {
	unpack.RegisterFormat("Petite", Petite{})
}

const (
	PETITE_VER_NONE = 0 + iota
	PETITE_VER_1
	PETITE_VER_2
	PETITE_VER_23
)

// Petite signatures and characteristics
var petiteSignatures = [][]byte{
	[]byte("petite"),
	[]byte("PETITE"),
	[]byte("Petite"),
}
...
func (Petite) Name() string {
	return "Petite"
}

// Detect checks if the file is packed with Petite
func (Petite) Detect(path string) int {
	peFile, err := pe.Open(path)
	if err != nil {
		return PETITE_VER_NONE
	}
	defer peFile.Close()

	// Check for Petite section
	hasPetiteSection := false
	for _, section := range peFile.Sections {
		...
		nameStr := string(name)
		if nameStr == ".petite" || nameStr == "petite" || nameStr == "PETITE" {
			hasPetiteSection = true
			break
		}
	}
	...
	// Check for Petite signature in file
	f, err := os.Open(path)
	if err != nil {
		return PETITE_VER_NONE
	}
	...
	// Read first 64KB to find Petite signature
	...
	// Check for Petite signatures
	for i, sig := range petiteSignatures {
		if bytes.Contains(buf[:n], sig) {
			return PETITE_VER_1 + i
		}
	}

	// Check entry point code for Petite characteristics
	ep := getEP(path)
	if ep > 0 {
		epbuff := make([]byte, 512)
		_, err = f.ReadAt(epbuff, int64(ep))
		if err == nil {
			// Petite often has specific patterns in entry point
			if hasPetitePattern(epbuff) {
				return PETITE_VER_1
			}
		}
	}
...
// hasPetitePattern checks for Petite-specific patterns in entry point code
func hasPetitePattern(epbuff []byte) bool {
	// Petite entry point often contains specific instruction sequences
	// Look for common patterns like PUSHAD, CALL, etc.
	if len(epbuff) < 10 {
		return false
	}
	...
	// Pattern 2: Specific byte sequences
	petitePatterns := [][]byte{
		{0x60, 0xE8},           // PUSHAD, CALL
		{0xE8, 0x00, 0x00, 0x00, 0x00}, // CALL with zero offset (placeholder)
	}

	for _, pattern := range petitePatterns {
...
// Unpack unpacks a Petite-packed PE file
func (Petite) Unpack(path string) (io.ReaderAt, error) {
	peFile, err := pe.Open(path)
	if err != nil {
		return nil, err
	}
	...
	oepRVA := findPetiteOEP(epbuff, peFile)
	if oepRVA == 0 {
	...
	// Fix import table
	fixPetiteImportTable(unpackedData, peFile, peSigOffset)
...
// findPetiteOEP finds OEP from Petite entry point code
func findPetiteOEP(epbuff []byte, peFile *pe.File) uint32 {
	// Petite stores OEP in various patterns
...
// fixPetiteImportTable attempts to fix the import table
func fixPetiteImportTable(unpackedData []byte, peFile *pe.File, peSigOffset int) {
...
func createTempFileFromData(data []byte) (io.ReaderAt, error) {
	tmpFile, err := os.CreateTemp("", "unpack_petite_*.exe")
	if err != nil {
		return nil, err
	}
