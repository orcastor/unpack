...
func init() {
	unpack.RegisterFormat("Armadillo", Armadillo{})
}

const (
	ARMADILLO_VER_NONE = 0 + iota
	ARMADILLO_VER_1
	ARMADILLO_VER_2
	ARMADILLO_VER_3
)

// Armadillo signatures and characteristics
var armadilloSignatures = [][]byte{
	[]byte("Armadillo"),
	[]byte("ARMADILLO"),
	[]byte("armadillo"),
	[]byte("!EPACK"),
...
func (Armadillo) Name() string {
	return "Armadillo"
}

// Detect checks if the file is packed with Armadillo
func (Armadillo) Detect(path string) int {
	peFile, err := pe.Open(path)
	if err != nil {
		return ARMADILLO_VER_NONE
	}
	defer peFile.Close()

	// Check for Armadillo section
	hasArmadilloSection := false
	for _, section := range peFile.Sections {
		...
		nameStr := string(name)
		if nameStr == ".armadillo" || nameStr == "armadillo" || nameStr == "ARMADILLO" ||
			nameStr == ".EPACK" || nameStr == "EPACK" {
			hasArmadilloSection = true
			break
		}
	}

	if hasArmadilloSection {
		return ARMADILLO_VER_1
	}

	// Check for Armadillo signature in file
	f, err := os.Open(path)
	if err != nil {
		return ARMADILLO_VER_NONE
	}
	...
	// Read first 64KB to find Armadillo signature
	...
	// Check for Armadillo signatures
	for i, sig := range armadilloSignatures {
		if bytes.Contains(buf[:n], sig) {
			return ARMADILLO_VER_1 + i
		}
	}

	// Check entry point code for Armadillo characteristics
	ep := getEP(path)
	if ep > 0 {
		epbuff := make([]byte, 512)
		_, err = f.ReadAt(epbuff, int64(ep))
		if err == nil {
			// Armadillo often has specific patterns in entry point
			if hasArmadilloPattern(epbuff) {
				return ARMADILLO_VER_1
			}
		}
	}
...
// hasArmadilloPattern checks for Armadillo-specific patterns in entry point code
func hasArmadilloPattern(epbuff []byte) bool {
	// Armadillo entry point often contains specific instruction sequences
	if len(epbuff) < 10 {
		return false
	}
	...
	// Pattern 2: Specific byte sequences common in Armadillo
	armadilloPatterns := [][]byte{
		{0x60, 0xE8},           // PUSHAD, CALL
		{0x9C, 0x60},           // PUSHFD, PUSHAD
		{0xE8, 0x00, 0x00, 0x00, 0x00}, // CALL with zero offset
		{0xEB, 0x00},           // JMP short (common in Armadillo)
	}

	for _, pattern := range armadilloPatterns {
...
// Unpack unpacks an Armadillo-packed PE file
func (Armadillo) Unpack(path string) (io.ReaderAt, error) {
	peFile, err := pe.Open(path)
	if err != nil {
		return nil, err
	}
	...
	oepRVA := findArmadilloOEP(epbuff, peFile)
	if oepRVA == 0 {
	...
	// Fix import table
	fixArmadilloImportTable(unpackedData, peFile, peSigOffset)
...
// findArmadilloOEP finds OEP from Armadillo entry point code
func findArmadilloOEP(epbuff []byte, peFile *pe.File) uint32 {
	// Armadillo stores OEP in various patterns
	...
	for i := 0; i < len(epbuff)-8; i++ {
		...
		// Pattern 4: LEA EAX, [OEP] followed by JMP/CALL EAX
		if i+7 <= len(epbuff) && epbuff[i] == 0x8D { // LEA
			if epbuff[i+1] == 0x05 || epbuff[i+1] == 0x0D { // LEA EAX, [imm32] or LEA ECX, [imm32]
				value := binary.LittleEndian.Uint32(epbuff[i+2:])
				if value >= 0x1000 && value < 0x100000 {
					// Check if next instruction is JMP EAX
					if i+7 <= len(epbuff) && epbuff[i+6] == 0xFF && epbuff[i+7] == 0xD0 {
						for _, section := range peFile.Sections {
							if section.SectionHeader.VirtualAddress <= value &&
								value < section.SectionHeader.VirtualAddress+section.SectionHeader.VirtualSize {
								return value
...
// fixArmadilloImportTable attempts to fix the import table
func fixArmadilloImportTable(unpackedData []byte, peFile *pe.File, peSigOffset int) {
...
func createTempFileFromData(data []byte) (io.ReaderAt, error) {
	tmpFile, err := os.CreateTemp("", "unpack_armadillo_*.exe")
	if err != nil {
		return nil, err
	}
